...
ServerTerminal st;
	...
	public void RpcMakeMeDead (byte cpos) {
		...
		goal=0;
		switch (cpos) {
		case 1:Instantiate(Resources.Load<GameObject>("explosion1"),transform.TransformPoint(c1.center),transform.rotation);break;
		case 2:Instantiate(Resources.Load<GameObject>("plasma_explosion"),transform.TransformPoint(c2.center),transform.rotation);break;
		case 3:Instantiate(Resources.Load<GameObject>("plasma_explosion"),transform.TransformPoint(c2.center),transform.rotation);break;
		case 4:Instantiate(Resources.Load<GameObject>("explosion1"),transform.TransformPoint(c2.center),transform.rotation);break;
		case 5:Instantiate(Resources.Load<GameObject>("explosion1"),transform.TransformPoint(c5.center),transform.rotation);break;
		case 6:Instantiate(Resources.Load<GameObject>("explosion1"),transform.TransformPoint(c6.center),transform.rotation);break;
		}
		GameObject wreck;
		breach wr_breach;
		Rigidbody r;
		if (cpos!=1) {wreck=Instantiate(ResLoad.simple_wreck,transform.TransformPoint(c1.center),transform.rotation) as GameObject;
			wreck.transform.localScale=a*0.9f;
		wreck.SetActive(true);
		wr_breach=wreck.AddComponent<breach>();
		wr_breach.damage_degree=2;
		r=wreck.GetComponent<Rigidbody>();
		r.velocity=rbody.velocity;
			r.angularVelocity=rbody.angularVelocity;}
		if (cpos!=2) {
		wreck=Instantiate(ResLoad.simple_wreck,transform.TransformPoint(c2.center),transform.rotation) as GameObject;
			wreck.transform.localScale=b*0.9f;
		wreck.SetActive(true);
		wr_breach=wreck.AddComponent<breach>();
		wr_breach.damage_degree=1;
		r=wreck.GetComponent<Rigidbody>();
		r.velocity=rbody.velocity;
		r.angularVelocity=rbody.angularVelocity;
		}
		if (cpos!=3) {
		wreck=Instantiate(ResLoad.simple_wreck,transform.TransformPoint(c3.center),transform.rotation) as GameObject;
			wreck.transform.localScale=c*0.9f;
		wreck.SetActive(true);
		wr_breach=wreck.AddComponent<breach>();
			wr_breach.further_breach=false;
		r=wreck.GetComponent<Rigidbody>();
		r.velocity=rbody.velocity;
		r.angularVelocity=rbody.angularVelocity;
		}
		if (c4&&cpos!=4) {
			wreck=Instantiate(ResLoad.simple_wreck,transform.TransformPoint(c4.center),transform.rotation) as GameObject;
			wreck.transform.localScale=d*0.9f;
			wreck.SetActive(true);
			wr_breach=wreck.AddComponent<breach>();
			wr_breach.further_breach=false;
			r=wreck.GetComponent<Rigidbody>();
		...
		if (c5&&cpos!=5) {
			wreck=Instantiate(ResLoad.simple_wreck,transform.TransformPoint(c5.center),transform.rotation) as GameObject;
			wreck.transform.localScale=e*0.9f;
			wreck.SetActive(true);
			wr_breach=wreck.AddComponent<breach>();
			wr_breach.damage_degree=1;
			r=wreck.GetComponent<Rigidbody>();
		...
		if (c6&&cpos!=6) {
			wreck=Instantiate(ResLoad.simple_wreck,transform.TransformPoint(c6.center),transform.rotation) as GameObject;
			wreck.transform.localScale=f*0.9f;
			wreck.SetActive(true);
			wr_breach=wreck.AddComponent<breach>();
			wr_breach.damage_degree=1;
			r=wreck.GetComponent<Rigidbody>();
			...
			speed=0;
			Instantiate(ResLoad.lf_explosion,transform.position,transform.rotation);
			if (srl.mission) 	GameObject.Find("menu").SendMessage("MissionFailed",SendMessageOptions.DontRequireReceiver);
		}
